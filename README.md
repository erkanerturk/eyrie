...
The **Simulate activity** switch (session-only: always off after a relaunch, like Keep Awake) runs `ActivitySimulator`: an idle threshold, not a fixed-period timer. Each check is scheduled for the moment the HID idle clock can cross the configured threshold (`interval - idleTime()`), and only when the user has genuinely been idle that long does it post a 1 pt synthetic mouse move-and-restore via `CGEvent` — so presence-based apps (Teams, Slack, …) don't flip to away, the nudge lands within a bounded delay of the last real input, and real activity always suppresses it. Posting needs the Accessibility (TCC) grant: the panel shows a caution line and prompts while trust is missing, and an untrusted simulator never reports itself as active (the loop keeps polling the grant each tick, so flipping it in System Settings lights things up without re-toggling). It runs with the panel closed, so `setModuleEnabled(_:)` stops/restores it and `shutdown()` kills it on quit.
...
Uses the same Apple Silicon route as MonitorControl/m1ddc: `DCPAVServiceProxy` IORegistry entries → `IOAVServiceCreateWithService` → raw DDC/CI packets via `IOAVServiceWriteI2C`/`ReadI2C` (resolved at runtime with `dlsym` in [DDCService.swift](Packages/DisplayKit/Sources/DisplayKit/DDCService.swift), so a macOS update that removes them only disables DDC). Displays are matched to services by comparing `CGDisplayCreateUUIDFromDisplayID` against the service's `EDID UUID` property, with a 1:1 fallback when exactly one of each is unmatched. All I2C traffic is serialized through the `DDCService` actor. Brightness is VCP code `0x10`; the max value is learned from the first successful read (assumed 100 if the monitor rejects reads).
...
- **IP rows** — local and external IP are masked until hovered (`•••.•••.•.50`: separators and the last group survive, so the width holds and networks stay tellable apart; copy still copies the real value). A mask rather than a blur, which pulled the eye. **Show IP addresses** (default on) hides both rows and skips the external lookup entirely.
